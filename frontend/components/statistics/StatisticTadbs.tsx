    ...
    const fetchAllData = async () => {
      try {
        const [countData, winLossData, ratingsData, historyData] =
          await Promise.all([
            UserService.getCountGames(),
            UserService.getMonthlyWinLoss(),
            UserService.getMonthlyAverageRatings(),
                    ...
                    {gameHistory.map((history, index) => (
                      ...
                      >
                        <div className="flex items-center gap-3">
                          <div
                            className={`flex items-center justify-center w-8 h-8 rounded-full ${history.winner === user?._id ? 'bg-green-100 text-green-600' : 'bg-red-100 text-red-600'}`}
                          >
                            {history.winner === user?._id ? (
